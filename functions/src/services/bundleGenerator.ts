...
1. MUSIC — a real piece of music. The URL MUST be a regular YouTube watch URL (https://www.youtube.com/watch?v=...). Do NOT use music.youtube.com (it requires a subscription). Search YouTube, find an actual video that plays the piece, and use its real watch URL. The video will be EMBEDDED on our page, so prefer videos that allow embedding: official artist channels, label channels, YouTube "Topic" auto-generated channels, well-established archival uploads (e.g. classical recordings, public performances). Avoid uploads that are likely to have embedding disabled or to be taken down (random user re-uploads of major-label pop, lyric-only videos from small channels).
